...
class Transformer():
    ...
    def voltage_callback(self,data):
        ...
        if voltage==self.voltage:
            ...
            print(f"self.init_voltage:{self.init_voltage}")
            exponential=math.exp(square)
            self.current=voltage/self.resistance+(self.init_voltage-voltage)/self.resistance*exponential

        else:
            self.init_voltage=self.current*self.resistance
        ...
        if self.current>0:   
            ...
            power=-self.current**2*self.resistance*self.coefficient
        self.velocity=(2*power*(time.time()-start_time)/self.total_mass+self.velocity**2)**(1/2)
        print(self.velocity)
